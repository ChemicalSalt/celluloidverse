...
const Farewell = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/dashboard/servers/${serverId}/plugins/farewell`,
        {
          method: "POST",
    ...
    dark:from-black dark:via-zinc-900 dark:to-black transition-colors duration-700">
      ...
      <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-center mb-10">
        <span className="bg-gradient-to-r from-zinc-800 via-zinc-600 to-zinc-800 dark:from-zinc-100 dark:via-zinc-400 dark:to-zinc-100 bg-clip-text text-transparent">
          Farewell Plugin Settings
        </span>
      </h1>

      <div className="max-w-2xl mx-auto flex flex-col gap-8 p-8 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-2xl shadow-xl border border-zinc-200 dark:border-zinc-800 transition-all duration-500">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Goodbye {user}, we’ll miss you in {server}!"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full p-3 border border-zinc-300 dark:border-zinc-700 rounded-lg bg-zinc-50 dark:bg-zinc-800 dark:text-white focus:ring-2 focus:ring-zinc-500 outline-none resize-none"
            placeholder="Bye {user}, hope to see you again!"
          />
        </div>
